...
public class GlobalSettings {
	...
	private static void initValues() {
		...
		if (!settings.contains(PREF_POWER_DISCONNECTTIMEOUT))
			editor.putInt(PREF_POWER_DISCONNECTTIMEOUT, 10);
	...
	public static void setAutoScrollTouchPointer(boolean scroll) {
		settings.edit().putBoolean(PREF_UI_AUTOSCROLLTOUCHPOINTER, scroll)
				.commit();
	}
	...
	public static void setAcceptAllCertificates(boolean accept) {
		settings.edit().putBoolean(PREF_SECURITY_ACCEPTALLCERTIFICATES, accept)
				.commit();
	}
	...
	public static void setDisconnectTimeout(int timeoutMinutes) {
		settings.edit().putInt(PREF_POWER_DISCONNECTTIMEOUT, timeoutMinutes)
				.commit();
	}
