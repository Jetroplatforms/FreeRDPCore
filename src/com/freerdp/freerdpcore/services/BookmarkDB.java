...
public class BookmarkDB extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase db) {
		String sqlScreenSettings = "CREATE TABLE tbl_screen_settings (" + ID
				+ " INTEGER PRIMARY KEY, " + "colors INTEGER DEFAULT 16, "
				+ "resolution INTEGER DEFAULT 0, " + "width, " + "height);";

		db.execSQL(sqlScreenSettings);

		String sqlPerformanceFlags = "CREATE TABLE tbl_performance_flags ("
				+ ID + " INTEGER PRIMARY KEY, " + "perf_remotefx INTEGER, "
				+ "perf_wallpaper INTEGER, " + "perf_theming INTEGER, "
		...
		// Insert a test entry
		String sqlInsertDefaultScreenEntry = "INSERT INTO tbl_screen_settings ("
				+ "colors, "
				+ "resolution, "
				+ "width, "
				+ "height) "
				+ "VALUES ( " + "32, 1, 1024, 768);";
		db.execSQL(sqlInsertDefaultScreenEntry);
		db.execSQL(sqlInsertDefaultScreenEntry);

		String sqlInsertDefaultPerfFlags = "INSERT INTO tbl_performance_flags ("
				+ "perf_remotefx, "
				+ "perf_wallpaper, "
				+ "perf_theming, "
		...
		String sqlInsertDefaultSessionEntry = "INSERT INTO tbl_manual_bookmarks ("
				+ "label, "
				+ "hostname, "
				+ "username, "
				+ "password, "
	...
	private String getManualBookmarksCreationString() {
		return ("CREATE TABLE IF NOT EXISTS tbl_manual_bookmarks ("
				+ ID
				+ " INTEGER PRIMARY KEY, "
				+ "label TEXT NOT NULL, "
				...
				+ "FOREIGN KEY(screen_settings) REFERENCES tbl_screen_settings("
				+ ID
				+ "), "
				+ "FOREIGN KEY(performance_flags) REFERENCES tbl_performance_flags("
				+ ID
				+ "), "
				+ "FOREIGN KEY(screen_3g) REFERENCES tbl_screen_settings("
				+ ID
				+ "), "
				+ "FOREIGN KEY(performance_3g) REFERENCES tbl_performance_flags("
				+ ID + ") "
	...
	public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
		...
		// run a table creation with if not exists (we are doing an upgrade, so
		// the table might
		...
		db.execSQL(getManualBookmarksCreationString());
		// get the intersection with the new columns, this time columns taken
		// from the upgraded table
